...
t_management(_Config) ->
    ...
    ok = emqx_acl_mnesia_cli:add_acl({clientid, <<"test_clientid">>}, <<"topic/A">>, sub, allow),
    ok = emqx_acl_mnesia_cli:add_acl({clientid, <<"test_clientid">>}, <<"topic/+">>, pub, deny),
    ok = emqx_acl_mnesia_cli:add_acl({username, <<"test_username">>}, <<"topic/A">>, sub, deny),
    ...
    deny  = emqx_access_control:check_acl(User1, publish,   <<"topic/B">>),
    deny  = emqx_access_control:check_acl(User2, subscribe, <<"topic/A">>),
    allow = emqx_access_control:check_acl(User2, publish,   <<"topic/B">>),
    allow = emqx_access_control:check_acl(User3, subscribe, <<"topic/A">>),
    deny  = emqx_access_control:check_acl(User3, publish,   <<"topic/B">>),
    deny  = emqx_access_control:check_acl(User3, subscribe, <<"topic/A/B">>),
    deny  = emqx_access_control:check_acl(User3, publish,   <<"topic/A/B">>),

    ok = emqx_acl_mnesia_cli:remove_acl({clientid, <<"test_clientid">>}, <<"topic/A">>),
    ok = emqx_acl_mnesia_cli:remove_acl({clientid, <<"test_clientid">>}, <<"topic/+">>),
    ok = emqx_acl_mnesia_cli:remove_acl({username, <<"test_username">>}, <<"topic/A">>),
